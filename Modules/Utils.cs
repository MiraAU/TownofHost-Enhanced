...
public static class Utils
{
    ...
    private static void BuildTargetIdTextInMeeting(PlayerControl seer, PlayerControl target, ref string TargetPlayerName)
    {
        if (Options.GuesserMode.GetBool())
        {
            if (seer.IsAlive() && target.IsAlive())
            {
                ...
                if (Options.CrewmatesCanGuess.GetBool() && seer.GetCustomRole().IsCrewmate() && !seer.Is(CustomRoles.JudgeCustom) && !seer.Is(CustomRoles.Inspector) && !seer.Is(CustomRoles.Lookout) && !seer.Is(CustomRoles.Swapper))
    ...
    public static bool DeathReasonIsEnable(this PlayerState.DeathReason reason, bool checkbanned = false)
    {
        ...
        return checkbanned ? !BannedReason(reason) : reason switch
        {
            ...
            PlayerState.DeathReason.Trialed => CustomRoles.JudgeCustom.IsEnable() 
